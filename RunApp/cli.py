...
def manual_login_or_register():
    for _ in range(3):
        ...
        choice = input('Would you like to register as a new user? (yes/no): '
            ).strip().lower()
        ...
        create_character_for_user(raw_phone, user['user_id'], party_id,
            user_name, character_name, role)
        print(
...
def choose_character(user_id):
    characters = query_db('SELECT * FROM characters WHERE user_id = ?', (
        user_id,))
    if not characters:
    ...
    if len(characters) == 1:
        ...
        for i, c in enumerate(characters, 1):
            print(
                f"{i}. {c['character_name']} the {c['role']} (Player name: {c['player_name']})"
                )
...
def choose_shop(conversation):
    ...
    # load and instantiate the agent with the conversation
...
def main():
    ...
    # 1) Create the Conversation up front
    convo = Conversation(character_id)

    # 2) Choose the shop, handing in our convo so the HandlerDebugMixin can use it
    shop_id, shop_name, agent = choose_shop(convo)
    if shop_id is None:
        print('[FATAL] Failed to load shop. Exiting.')
        return

    # 3) Track and fetch visit info
    ...
    # 5) Hand off to the ConversationService
