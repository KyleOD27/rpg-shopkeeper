...
def handle_admin_command(admin_input):
    ...
    if command == 'shutdown':
        ...
        return ('✅ System running. Debug mode is ON.' if RuntimeFlags.
            DEBUG_MODE else '✅ System running. Debug mode is OFF.')
    elif command == 'log':
        logs = query_db(
        ...
        return '\n'.join([
            f"{log['timestamp']} - {log['action']}: {log['details']}" for
            log in logs])
    elif command == 'restart':
        execute_db('INSERT INTO system_logs (action, details) VALUES (?, ?)',
            ('RESTART', 'Admin restart requested.'))
        return (
            '🔁 Restart requested (stub). You must restart the app manually or via process manager.'
            )
    ...
    return """Unknown admin command. Available commands:
- admin shutdown
- admin status
- admin log
- admin restart
