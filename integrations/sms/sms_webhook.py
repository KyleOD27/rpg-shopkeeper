...
logging.basicConfig(level=logging.DEBUG if RuntimeFlags.DEBUG_MODE else
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler('sms_debug.log',
    mode='a')])
...
def sms():
    try:
        ...
        if not sender or not body:
            app.logger.warning(
                f"⚠️ Missing sender or body! Sender: '{sender}', Body: '{body}'"
                )
            return Response(
                '<Response><Message>Invalid message received.</Message></Response>'
                , mimetype='application/xml')
        app.logger.info(
            f"📩 SMS received from {sender}: '{body}' (length: {len(body)})")
        try:
        ...
        return Response(
            '<Response><Message>Major error occurred. Please try again later.</Message></Response>'
            , mimetype='application/xml')


def send_startup_sms():
    try:
        params = {'body': '📦 Shopkeeper is online!', 'to': TO_NUMBER}
        if MESSAGING_SERVICE_SID:
            ...
            raise ValueError(
                '❌ FROM_SMS_NUMBER or MESSAGING_SERVICE_SID required')
