...
def add_character_to_party(phone_number, party_id, player_name, character_name=None, role=None):
    # Get or create user by phone number
    user = get_user_by_phone(phone_number)
    if not user:
        print(f"[INFO] No user found with phone {phone_number}. Creating one.")
        execute_db("INSERT INTO users (phone_number, user_name) VALUES (?, ?)", (phone_number, player_name))
        user = get_user_by_phone(phone_number)

    user_id = user["user_id"]

    # Check if character already exists in this party
    sql_check = "SELECT character_id FROM characters WHERE LOWER(player_name) = ? AND party_id = ?"
    ...
    new_character = query_db("SELECT character_id FROM characters WHERE LOWER(player_name) = ? AND party_id = ?",
...
def get_all_owned_characters(dm_id):
    ...
    """
    # First, get all party IDs that the DM owns
    sql_parties = "SELECT party_id FROM parties WHERE dm_id = ?"
    party_ids = query_db(sql_parties, (dm_id,))

    # If the DM has no parties, return an empty list
    if not party_ids:
        return []

    # Get characters associated with those parties
    sql_characters = """
        SELECT * FROM characters WHERE party_id IN ({})
    """.format(','.join(str(p['party_id']) for p in party_ids))  # Dynamically build the IN clause
