...
def buy_item(party_id, player_id, item_name):
    ...
    if current_gold < price:
        return {'success': False, 'message':
            f"Not enough gold. You have {current_gold} gold, but {item['item_name']} costs {price} gold."
            }
    ...
    execute_db(
        ...
        VALUES (?, ?, 'BUY', ?, ?, ?, ?)
    """
        , (party_id, player_id, item['item_name'], -price, new_gold,
        f"Purchased {item['item_name']}"))
    return {'success': True, 'message':
        f"You bought {item['item_name']} for {price} gold. Party gold remaining: {new_gold}."
        , 'cost': price}
...
def sell_item(party_id, player_id, item_name, estimated_value):
    ...
    execute_db(
        ...
        VALUES (?, ?, 'SELL', ?, ?, ?, ?)
    """
        , (party_id, player_id, item_name, gold_earned, new_gold,
        f'Sold {item_name}'))
    update_reputation(party_id, 1)
    return {'success': True, 'message':
        f'You sold {item_name} for {gold_earned} gold. Party gold now: {new_gold}.'
        }
...
def haggle(party_id, item_name):
    ...
    if roll == 1:
        ...
        message += (
            f'Disaster! Grizzlebeard raises the price to {final_price} gold!')
    elif total <= 9:
        ...
        message += (
            f'Minor success. Grizzlebeard offers it for {final_price} gold.')
    elif total <= 19:
        final_price = int(base_price * 0.8)
        message += (
            f'Good success! Grizzlebeard offers it for {final_price} gold.')
    else:
        final_price = int(base_price * 0.7)
        message += (
            f'Critical success! Grizzlebeard is impressed. Price is {final_price} gold.'
            )
...
def deposit_gold(party_id, player_id, amount):
    ...
    execute_db(
        ...
        VALUES (?, ?, 'DEPOSIT', ?, ?, ?)
    """
        , (party_id, player_id, amount, new_gold,
        f'Deposited {amount} gold to the Party Stash'))
    return {'success': True, 'message':
        f'Deposited {amount} gold. Party gold now: {new_gold}.'}
...
def withdraw_gold(party_id, player_id, amount):
    ...
    if party['party_gold'] < amount:
        return {'success': False, 'message':
            f"Not enough gold. Party has {party['party_gold']} gold."}
    new_gold = party['party_gold'] - amount
    update_party_gold(party_id, new_gold)
    execute_db(
        """
    ...
    """
        , (party_id, player_id, -amount, new_gold,
        f'Withdrew {amount} gold from the Party Stash'))
    return {'success': True, 'message':
        f'Withdrew {amount} gold. Party gold now: {new_gold}.'}
