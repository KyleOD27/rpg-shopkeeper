...
def handle_dm_command(party_id, player_id, player_input, party_data=None):
    ...
    if command == 'add_gold':
        ...
        return (f"[DM] New party '{name}' created with ID '{new_id}'" if
            new_id else '❌ Failed to create party.')
        ...
        return (f"[DM] User '{name}' created." if user else
            '❌ Failed to register user.')
    elif command == 'new_char':
        if len(parts) < 6:
            return (
                'Usage: dm new_char <+44...> <party_id> <player name> <char name> <role>'
                )
        phone = parts[2]
        party_id = parts[3]
        player_name = parts[4]
        ...
        create_character_for_user(phone, user['user_id'], party_id,
            player_name, char_name, role)
        ...
        if not user:
            return '❌ User not found.'
        execute_db('UPDATE users SET user_name = ? WHERE user_id = ?', (
        ...
        party_id = parts[2]
        new_name = ' '.join(parts[3:])
        execute_db('UPDATE parties SET party_name = ? WHERE party_id = ?',
            (new_name, party_id))
        ...
        return (
            f"[DM] Character {character_id} updated to '{new_name}' the {new_role}'."
            )
        ...
        user_list = '\n'.join([
            f"{user['user_name']} ({user['phone_number']})" for user in users])
        return f'[DM] Users:\n{user_list}'
        ...
        char_list = '\n'.join([
            f"{char['character_name']} ({char['role']}) - Player: {char['player_name']}"
             for char in characters])
        return f'[DM] Characters:\n{char_list}'
    return """Unknown DM command. Available commands:
- dm add_gold <amount>
- dm new_party <name>
